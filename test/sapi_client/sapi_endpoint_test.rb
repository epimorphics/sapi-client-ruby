    ...
    describe 'SapiEndpoint' do
      ...
      describe '#view' do
        it 'should look up a view by name' do
          mock_views = mock('view_registry')
          mock_views.expects(:named_view).with('womble').returns(SapiClient::View.new('name' => 'wombleView')).at_least(1)

          ep = SapiClient::SapiEndpoint.new('http://foo.bar', { 'type' => 'item', 'views' => { 'default' => 'womble' } }, view_registry: mock_views)
          v = ep.view('default')
      ...
      describe '#resource_type' do
        it 'should report the default resource type' do
          mock_views = mock('view_registry')
          mock_views.expects(:named_view).with('womble').returns(SapiClient::View.new('name' => 'wombleView', 'view' => { 'class' => ':Womble' }))
      ...
      describe '#resource_type_wrapper_class' do
        it 'should report the wrapper class for the resource type, if defined' do
          mock_views = mock('view_registry')
          mock_views.expects(:named_view).with('womble').returns(SapiClient::View.new('name' => 'wombleView', 'view' => { 'class' => ':MockWomble' })).at_least(1)
        ...
        it 'should return the default class with no error if there is no explicit wrapper class defined' do
          mock_views = mock('view_registry')
          mock_views.expects(:named_view).with('womble').returns(SapiClient::View.new('name' => 'wombleView', 'view' => { 'class' => ':Wombles' })).at_least(1)
